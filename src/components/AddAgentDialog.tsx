...
import { Folder, ModelConfig, Agent } from '@/types';
...
export const AddAgentDialog = ({ open: openProp, onOpenChange, folderId: propFolderId, onSuccess }: { open?: boolean, onOpenChange?: (open: boolean) => void, folderId?: string, onSuccess?: () => void }) => {
  ...
  // New states for system prompt dialog
  ...
  const handleCreateAgent = async () => {
    ...
    try {
      ...
      // Set the created agent and show the system prompt dialog
    ...
    <>
      ...
      <AddSystemPromptDialog 
        ...
        onSuccess={() => {
          toast({
            title: "Hoàn tất!",
            description: "Agent và System Prompt đã được tạo thành công.",
          });
        }}
      />
    </>
