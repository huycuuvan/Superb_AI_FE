...
const SuperbAiLogo: React.FC<{ size?: "sm" | "md" | "lg" }> = ({
  size = "md",
}) => {
  const sizeClasses = {
    sm: "text-xl",
  ...
  return (
    <Link to="/" className="flex items-center space-x-2.5 group relative z-10">
      <div
        className={`p-2 bg-gradient-to-br from-purple-500 via-pink-500 to-rose-500 rounded-lg shadow-md group-hover:opacity-90 transition-opacity`}
      >
        <svg
      ...
      <span
        className={`font-bold ${sizeClasses[size]} text-white group-hover:opacity-80 transition-opacity`}
      >
        Superb AI
      </span>
    </Link>
      ...
      >
        ...
        >
          <CardHeader className="space-y-1.5 p-6 sm:p-8 border-b border-white/20 dark:border-zinc-700/50">
            <CardTitle className="text-2xl sm:text-3xl font-bold text-center text-slate-800 dark:text-zinc-100">
              Welcome Back!
            </CardTitle>
          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-6 p-6 sm:p-8">
              <div className="space-y-1.5">
                <Label
                  htmlFor="email"
                  className="font-medium text-sm text-slate-700 dark:text-zinc-300"
                >
                  {t("auth.email")}
                </Label>
                <Input
              ...
              {(error || googleError) && (
                <div className="text-red-500 dark:text-red-400 text-sm text-center pt-1 font-medium">
                  {error || googleError}
                </div>
              )}
              ...
              <Button
                type="submit"
                className="w-full bg-primary text-white hover:bg-primary-gradient shadow-lg hover:shadow-primary/40 dark:shadow-primary/80"
                size="lg"
                disabled={loading}
              >
